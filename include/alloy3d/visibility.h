...
public:
  ...
  bool intersects(const Bounds3D &bounds,
                  simd_float4x4 model = matrix_identity_float4x4) const
  {
    if (!bounds.isValid()) return true;
    auto rows = simd_transpose(simd_mul(clip_, model));
    std::array<simd_float4, 6> planes = {rows.columns[3]+rows.columns[0], rows.columns[3]-rows.columns[0],
        rows.columns[3]+rows.columns[1], rows.columns[3]-rows.columns[1], rows.columns[2], rows.columns[3]-rows.columns[2]};
    ...
    for (auto p : planes)
    {
      ...
      if (distance + radius < -1e-5f * (1 + std::abs(distance) + radius)) return false;
    }
    return true;
  }
};
