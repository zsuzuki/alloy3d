                                                 ...
                                                 simd_float3                direction)
{
  ...
  if (!settings.bounds.isValid() || settings.resolution < 64 || settings.resolution > 4096 ||
      ...
      !std::isfinite(settings.filterRadius) || settings.filterRadius < 0 ||
      settings.filterRadius > 4)
  ...
  if (settings.stabilize)
  {
    ...
    auto        snapped = center + right * (std::round(x / texel) * texel - x) +
                          up * (std::round(y / texel) * texel - y);
