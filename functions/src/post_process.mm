...
PostProcess::PostProcess(id<MTLDevice> device, id<MTLLibrary> library, MTLPixelFormat colorFormat,
                         MTLPixelFormat depthFormat, NSUInteger samples, bool sceneEffects)
    : device_(device), samples_(samples), sceneEffects_(sceneEffects)
{
...
PostProcess::~PostProcess()
{
  ...
  {
    [page.color release];
    [page.multisample release];
    [page.bloom release];
    [page.bloomScratch release];
    [page.depth release];
    [page.sceneColor release];
    [page.sceneDepth release];
    [page.volume release];
  }
  [volumePipeline_ release];
  [depthPipeline_ release];
  [pipeline_ release];
  [depthState_ release];
  [library_ release];
  [bloomExtract_ release];
  [bloomBlur_ release];
}
void PostProcess::set(const PostProcessing3D &settings)
{
  if (!std::isfinite(settings.exposure) || settings.exposure < 0 || settings.exposure > 16 ||
      ...
      !std::isfinite(settings.bloom.strength) || settings.bloom.strength < 0 ||
      settings.bloom.strength > 4 || !std::isfinite(settings.bloom.threshold) ||
      settings.bloom.threshold < 0 || settings.bloom.threshold > 64 ||
      !std::isfinite(settings.volumetric.strength) || settings.volumetric.strength < 0 ||
      settings.volumetric.strength > 4 || !std::isfinite(settings.volumetric.density) ||
      settings.volumetric.density < 0 || settings.volumetric.density > 1 ||
      !std::isfinite(settings.volumetric.baseHeight) ||
      !std::isfinite(settings.volumetric.falloff) || settings.volumetric.falloff < 0 ||
      settings.volumetric.falloff > 10 || !std::isfinite(settings.volumetric.maxDistance) ||
      settings.volumetric.maxDistance <= 0 || settings.volumetric.maxDistance > 1000 ||
      !std::isfinite(settings.volumetric.anisotropy) ||
      std::abs(settings.volumetric.anisotropy) > .9f || settings.volumetric.steps < 8 ||
      settings.volumetric.steps > 64 || (settings.volumetric.strength > 0 && !sceneEffects_) ||
      !std::isfinite(settings.bloom.radius) || settings.bloom.radius < 1 ||
      settings.bloom.radius > 32)
    throw std::invalid_argument(
        "Invalid post-processing settings or volumetric lighting without sceneEffects");
  settings_ = settings;
}
MTLRenderPassDescriptor *PostProcess::begin(MTLRenderPassDescriptor *output, NSUInteger index)
{
  ...
  if (!target || !target.width || !target.height)
    throw std::invalid_argument("HDR requires a nonempty output target");
  if (!page.color || page.color.width != target.width || page.color.height != target.height ||
      page.releasePending)
  {
    ...
    [page.bloom release];
    [page.bloomScratch release];
    page.bloom = page.bloomScratch = nil;
    page.color                     = color;
    page.multisample               = multisample;
    page.releasePending            = false;
    [page.volume release];
    page.volume = nil;
    [page.depth release];
    [page.sceneColor release];
    [page.sceneDepth release];
    page.depth = page.sceneColor = page.sceneDepth = nil;
    if (sceneEffects_)
    {
      ...
      desc.textureType = samples_ > 1 ? MTLTextureType2DMultisample : MTLTextureType2D;
      desc.sampleCount = samples_;
      desc.usage       = MTLTextureUsageRenderTarget | MTLTextureUsageShaderRead;
      page.depth       = [device_ newTextureWithDescriptor:desc];
      if (!page.sceneColor || !page.sceneDepth || !page.depth)
        throw std::bad_alloc();
    }
  }
  ...
  if (sceneEffects_)
  {
    ...
    color.storeAction =
        page.multisample ? MTLStoreActionStoreAndMultisampleResolve : MTLStoreActionStore;
...
MTLRenderPassDescriptor *PostProcess::transparentPass(MTLRenderPassDescriptor *output,
                                                      NSUInteger               index)
{
  auto                    &page  = pages_.at(index);
  MTLRenderPassDescriptor *pass  = [[output copy] autorelease];
...
void PostProcess::captureScene(id<MTLCommandBuffer> commands, NSUInteger index,
                               const CameraData &camera)
{
  ...
  if (samples_ == 1)
  {
    ...
    [blit copyFromTexture:page.color toTexture:page.sceneColor];
    [blit endEncoding];
  }
  if (!depthPipeline_)
  {
    NSError *error = nil;
    depthPipeline_ = [device_
        newComputePipelineStateWithFunction:[[library_
                                                newFunctionWithName:samples_ > 1
                                                                        ? @"resolveSceneDepthMS"
                                                                        : @"resolveSceneDepth"]
                                                autorelease]
                                      error:&error];
  ...
  } uniforms{simd_inverse(camera.getProjectionMatrix()),
             {camera.getProjectionMode() == ProjectionMode::Identity ? 1.f : -1.f, 0, 0, 0}};
  auto encoder = [commands computeCommandEncoder];
  [encoder setComputePipelineState:depthPipeline_];
  [encoder setTexture:page.depth atIndex:0];
  [encoder setTexture:page.sceneDepth atIndex:1];
  [encoder setBytes:&uniforms length:sizeof(uniforms) atIndex:0];
  [encoder dispatchThreads:MTLSizeMake(page.color.width, page.color.height, 1)
...
void PostProcess::prepare(id<MTLCommandBuffer> commands, NSUInteger index)
{
  ...
  if (!bloomExtract_)
  {
    NSError *error = nil;
    auto     extract =
        [device_ newComputePipelineStateWithFunction:[[library_ newFunctionWithName:@"bloomExtract"]
                                                         autorelease]
                                               error:&error];
    auto blur =
        [device_ newComputePipelineStateWithFunction:[[library_ newFunctionWithName:@"bloomBlur"]
                                                         autorelease]
                                               error:&error];
    if (!extract || !blur)
    {
      [extract release];
      [blur release];
      throw std::runtime_error("Bloom pipeline creation failed");
    }
    bloomExtract_ = extract;
    bloomBlur_    = blur;
  }
  if (!page.bloom)
  {
    auto desc = [MTLTextureDescriptor texture2DDescriptorWithPixelFormat:MTLPixelFormatRGBA16Float
                                                                   width:(page.color.width + 1) / 2
                                                                  height:(page.color.height + 1) / 2
                                                               mipmapped:NO];
    desc.storageMode = MTLStorageModePrivate;
    desc.usage       = MTLTextureUsageShaderRead | MTLTextureUsageShaderWrite;
    auto bloom       = [device_ newTextureWithDescriptor:desc],
         scratch     = [device_ newTextureWithDescriptor:desc];
    if (!bloom || !scratch)
    {
      [bloom release];
      [scratch release];
    ...
    page.bloom        = bloom;
    page.bloomScratch = scratch;
  }
  auto dispatch = [&](id<MTLComputePipelineState> pipeline,
                      id<MTLTexture>
                          input,
                      id<MTLTexture>
                                  output,
...
{
  simd_float4 parameters = {settings_.exposure,
                            float(settings_.toneMapping),
                            settings_.bloom.strength,
                            settings_.volumetric.strength > 0 ? 1.f : 0.f};
  ...
  [encoder setFragmentTexture:settings_.bloom.strength > 0 ? pages_.at(page).bloom
                                                           : pages_.at(page).color
                      atIndex:1];
  [encoder
      setFragmentTexture:pages_.at(page).volume ? pages_.at(page).volume : pages_.at(page).color
                 atIndex:2];
  [encoder setFragmentTexture:pages_.at(page).sceneDepth ? pages_.at(page).sceneDepth
                                                         : pages_.at(page).color
                      atIndex:3];
                                 ...
                                 simd_float4x4 shadow, simd_float4 parameters, id<MTLTexture> map)
{
  ...
  volumeUniforms_.marchParameters   = {
      s.maxDistance,
      float(s.steps),
...
{
  if (settings_.volumetric.strength <= 0)
    return;
  if (!volumeShadow_)
    throw std::logic_error("Volumetric scene lighting was not configured");
  auto &page = pages_.at(index);
  if (!volumePipeline_)
  {
    NSError *error = nil;
    volumePipeline_ =
        [device_ newComputePipelineStateWithFunction:[[library_ newFunctionWithName:@"volumeLight"]
                                                         autorelease]
                                               error:&error];
    if (!volumePipeline_)
      throw std::runtime_error(error.localizedDescription.UTF8String);
  ...
  if (!page.volume)
  {
    ...
    desc.storageMode = MTLStorageModePrivate;
    desc.usage       = MTLTextureUsageShaderRead | MTLTextureUsageShaderWrite;
    page.volume      = [device_ newTextureWithDescriptor:desc];
    if (!page.volume)
      throw std::bad_alloc();
  }
  auto encoder  = [commands computeCommandEncoder];
  encoder.label = @"Alloy3D volumetric light";
  [encoder setComputePipelineState:volumePipeline_];
  [encoder setTexture:page.sceneDepth atIndex:0];
  [encoder setTexture:volumeShadow_ atIndex:1];
  [encoder setTexture:page.volume atIndex:2];
  [encoder setBytes:&volumeUniforms_ length:sizeof(volumeUniforms_) atIndex:0];
  [encoder dispatchThreads:MTLSizeMake(page.volume.width, page.volume.height, 1)
      threadsPerThreadgroup:MTLSizeMake(8, 8, 1)];
  [encoder endEncoding];
  volumeShadow_ = nil;
}
void PostProcess::releaseUnusedMemory()
{
  for (auto &page : pages_)
    page.releasePending = true;
}
size_t PostProcess::bytes() const
{
  size_t total = 0;
  for (const auto &page : pages_)
    total += page.color.allocatedSize + page.multisample.allocatedSize + page.bloom.allocatedSize +
             page.bloomScratch.allocatedSize + page.depth.allocatedSize +
             page.sceneColor.allocatedSize + page.sceneDepth.allocatedSize +
             page.volume.allocatedSize;
  return total;
}
bool PostProcess::releasePending() const
{
  for (const auto &page : pages_)
    if (page.releasePending)
      return true;
  return false;
}
} // namespace alloy3d::metal
