...
{
  if (material.wind.z <= 0)
    return;
  float height = (position.y - material.windShape.x) * material.windShape.y;
  float u      = saturate(height);
  float wave =
      (sin(material.wind.w + phase) + .35f * sin(material.wind.w * 1.7777778f + phase * 1.7f)) /
      1.35f;
                       ...
                       uint                            vertexId [[vertex_id]])
{
  ...
  o.viewDepth =
      cameraData.fogColorAndEnabled.w != 0 ? -(cameraData.worldTransform * position).z : 0;
  ...
  if ((material.highlight.x > 0 && material.highlight.z != 0 && material.parameters.w == 0) ||
      material.normalMapping.x != 0 || material.transmission.w > 0 || material.softDistance > 0 ||
      material.screenSurface.x > 0 || material.screenSurface.z > 0 ||
      cameraData.heightFogColorDensity.w > 0)
#endif
...
inline void ApplyModelCoverage(float2 pixel, float2 interval)
{
  ...
  if (threshold < interval.x || threshold >= interval.y)
    discard_fragment();
}

inline half3 SceneSurfaceColor(half3 color, float2 pixel, float3 position, float3 normal,
                               constant MaterialUniforms &m, device const Uniforms &camera,
                               texture2d<half> scene, texture2d<float> depth)
{
  ...
  {
    float2 offset =
        uv + float2(normal.x, -normal.y) * m.screenSurface.y * camera.sceneParameters.yz;
    // Reject foreground objects and offscreen displacement; use the undistorted background.
    if (any(offset < 0) || any(offset > 1) ||
        depth.sample(depthSampler, offset).r < ownDepth - .001f)
      offset = uv;
    color = mix(color, scene.sample(linearSampler, offset).rgb, half(m.screenSurface.x));
  }
  if (m.screenSurface.z > 0 && dot(normal, normal) > .5f)
  {
    float3 incident = camera.heightFogParameters.w > 0 ? normalize(position)
                                                       : float3(0, 0, camera.sceneParameters.w);
    ...
    for (uint i = 1; i <= 32; ++i)
    {
      ...
      if (any(hit <= 0) || any(hit >= 1) || clip.z < 0 || clip.z > clip.w)
        break;
      ...
      if (gap >= 0 && gap < m.screenThickness && previous <= 0)
      {
        ...
        color         = mix(color,
                            scene.sample(linearSampler, hit).rgb,
                            half(m.screenSurface.z * fresnel * edge * (1 - float(i) / 33.f)));
        break;
      }
      previous = gap;
    }
...
fragment half4 modelFrag3d(
    v2f in [[stage_in]], bool frontFacing [[front_facing]],
    device const Uniforms          &cameraData [[buffer(1)]],
    constant MaterialUniforms      &material [[buffer(5)]],
    texture2d<half, access::sample> tex [[texture(0)]], depth2d<float> shadowMap [[texture(1)]],
    sampler colorSampler [[sampler(0)]], texture2d<half> normalMap [[texture(2)]],
    texture2d<half> roughnessMap [[texture(3)]], texture2d<half> occlusionMap [[texture(4)]],
    texture2d<half> sceneColor [[texture(5)]], texture2d<float> sceneDepth [[texture(6)]]
...
)
{
  ...
  if (material.parameters.x == 1 && AlphaCoverage)
  {
    ...
    maskCoverage =
        saturate((alpha - material.parameters.y) / max(fwidth(alpha), 1.f / 255.f) + .5f);
    if (maskCoverage <= 0)
      discard_fragment();
  }
  if (material.parameters.x != 2)
    baseColor.a = half(maskCoverage);
  ...
  if (material.softDistance > 0 && cameraData.sceneParameters.x > 0)
  {
    constexpr sampler depthSampler(coord::normalized, address::clamp_to_edge, filter::nearest);
    float             opaqueDepth =
        sceneDepth.sample(depthSampler, in.position.xy * cameraData.sceneParameters.yz).r;
    baseColor.a *= half(saturate((opaqueDepth - in.viewPosition.z * cameraData.sceneParameters.w) /
                                 material.softDistance));
  }
  ...
  if (material.normalMapping.x != 0 && !unlit)
  {
    ...
    if (handedness != 0 && dot(t, t) > .5f)
    {
      ...
      float3 result =
          UnitModelNormal(t * mapped.x + cross(n, t) * handedness * mapped.y + n * mapped.z);
      if (dot(result, result) > .5f)
        n = result;
    }
  }
  if (!front)
  ...
  if (material.surfaceDetail.x != 0 && !unlit)
  {
    ...
    roughness =
        saturate(material.surfaceDetail.y *
                 ((flags & 1) ? float(roughnessMap.sample(colorSampler, in.texcoord).g) : 1.f));
    if (flags & 2)
      occlusion = mix(
          1.f, float(occlusionMap.sample(colorSampler, in.texcoord).r), material.surfaceDetail.z);
  }
#ifdef ALLOY3D_CUSTOM_MATERIAL
  ModelMaterial        input{float3(baseColor.rgb), n, roughness, occlusion, float3(0)};
  ModelMaterialContext context{in.texcoord,
                               in.viewPosition,
                               material.highlight.z != 0 ? UnitModelNormal(-in.viewPosition)
                                                         : float3(0, 0, 1),
                               unlit};
  ...
  if (all(isfinite(result.baseColor)))
    baseColor.rgb = half3(clamp(result.baseColor, 0.f, 64.f));
  if (any(result.normal != n) && all(isfinite(result.normal)) &&
      dot(result.normal, result.normal) > 1e-8f)
    n = UnitModelNormal(result.normal);
  if (isfinite(result.roughness))
  {
    modifiedRoughness = result.roughness != roughness;
    roughness         = saturate(result.roughness);
  }
  if (isfinite(result.occlusion))
    occlusion = saturate(result.occlusion);
  if (all(isfinite(result.emissive)))
    emissive = half3(clamp(result.emissive, 0.f, 64.f));
#endif
  ...
  if (!unlit && material.highlight.x > 0 && diffuse > 0 && shadow > 0)
  {
    ...
    if (dot(n, v) > 0 && h2 > 1e-8f)
    {
      ...
      float exponent = material.surfaceDetail.x != 0 || modifiedRoughness
                           ? clamp(2.f / max(roughness * roughness, .015f) - 2.f, 1.f, 128.f)
  ...
  if (cameraData.sceneParameters.x > 0 &&
      (material.screenSurface.x > 0 || material.screenSurface.z > 0))
    output.rgb = SceneSurfaceColor(output.rgb,
                                   in.position.xy,
                                   in.viewPosition,
                                   n,
                                   material,
...
vertex v2f textVert3d(device const VertexData3D *vertexData [[buffer(0)]],
                      device const Uniforms &cameraData [[buffer(1)]], uint vertexId [[vertex_id]])
{
  v2f o{};
