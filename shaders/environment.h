...
inline half4 ApplyFog(half4 color, float viewDepth, float3 viewPosition, device const Uniforms &camera)
{
  ...
  if (camera.heightFogColorDensity.w > 0)
  {
    ...
    float distance = camera.heightFogParameters.w != 0 ? length(viewPosition) : max(0.f, -viewPosition.z);
    float start = dot(origin, camera.heightFogWorldUp.xyz) + camera.heightFogWorldUp.w - camera.heightFogParameters.x;
    float end = dot(viewPosition, camera.heightFogWorldUp.xyz) + camera.heightFogWorldUp.w - camera.heightFogParameters.x;
    ...
    float density = camera.heightFogColorDensity.w * exp(clamp(-min(start, end) * camera.heightFogParameters.y, -50.f, 50.f));
    float t = min(1.f - exp(-min(density * distance * integral, 80.f)), camera.heightFogParameters.z);
