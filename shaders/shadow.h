...
inline half ShadowVisibility(float4 position, float4 parameters, depth2d<float> shadowMap)
{
  ...
  if (parameters.z <= 0) return half(shadowMap.sample_compare(compareSampler, uv, ndc.z - parameters.y));
  float visibility = 0;
  for (int y=-1; y<=1; ++y)
    for (int x=-1; x<=1; ++x)
    {
      float2 tap = uv + float2(x,y)*parameters.z*parameters.w;
      visibility += any(tap < 0) || any(tap > 1) ? 1.f : shadowMap.sample_compare(compareSampler,tap,ndc.z-parameters.y);
    }
