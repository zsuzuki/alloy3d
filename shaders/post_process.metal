...
fragment half4 toneMapFragment(PostVertex in [[stage_in]], texture2d<half> hdr [[texture(0)]],
                               ...
                               texture2d<half>  volume [[texture(2)]],
                               texture2d<float> sceneDepth [[texture(3)]])
{
  constexpr sampler s(coord::normalized, address::clamp_to_edge, filter::linear);
  half4             sample = hdr.sample(s, in.uv);
  float3            color  = float3(sample.rgb);
  if (parameters.z > 0)
    color += float3(bloom.sample(s, in.uv).rgb) * parameters.z;
  if (parameters.w > 0)
  {
    ...
    for (int y = 0; y < 2; ++y)
      ...
      {
        uint2  q      = uint2(clamp(
            base + int2(x, y), int2(0), int2(volume.get_width() - 1, volume.get_height() - 1)));
...
{
  if (id.x >= bloom.get_width() || id.y >= bloom.get_height())
    return;
  float3 sum = 0;
  for (uint y = 0; y < 2; ++y)
    for (uint x = 0; x < 2; ++x)
    {
      float3 color = max(float3(scene
                                    .read(min(id * 2 + uint2(x, y),
                                              uint2(scene.get_width() - 1, scene.get_height() - 1)))
                                    .rgb),
                         0.f);
      ...
      float soft = clamp(brightness - p.x + knee, 0.f, 2 * knee);
                      ...
                      constant float4 &p [[buffer(0)]], uint2 id [[thread_position_in_grid]])
{
  ...
  for (int i = 1; i <= 4; ++i)
    color += (float3(input.sample(s, uv + step * float(i)).rgb) +
              float3(input.sample(s, uv - step * float(i)).rgb)) *
             weights[i];
  output.write(half4(half3(color), 0), id);
}

struct SceneDepthUniforms
{
...
kernel void resolveSceneDepth(depth2d<float, access::read>    source [[texture(0)]],
                              ...
                              constant SceneDepthUniforms    &u [[buffer(0)]],
                              uint2                           p [[thread_position_in_grid]])
{
  uint2 size(target.get_width(), target.get_height());
  if (any(p >= size))
    return;
                                ...
                                uint2                           p [[thread_position_in_grid]])
{
  ...
  if (any(p >= size))
    return;
  float d = 1;
  for (uint i = 0; i < source.get_num_samples(); ++i)
    d = min(d, source.read(p, i));
                        ...
                        uint2                          p [[thread_position_in_grid]])
{
  ...
  for (uint i = 0; i < uint(u.marchParameters.y); ++i)
  {
    ...
    float  density =
        u.densityParameters.x *
        exp(clamp(-u.densityParameters.z * (point.y - u.densityParameters.y), -16.f, 8.f));
    float extinction = exp(-density * step);
    float visibility =
        float(ShadowVisibility(u.shadowTransform * float4(point, 1), u.shadowParameters, shadow));
