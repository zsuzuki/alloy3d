    ...
    try
    {
        ...
        return 77;
      {
        ...
        for (size_t i = 0; i < instances.size(); ++i)
          instances[i] = {
              {float(i % 20) * .07f - .7f, float(i / 20) * .08f - .6f, .2f + float(i % 17) * .03f},
              {0, 0, float(i) * .03f},
              {i % 2 ? -.16f : .16f, .16f, 1},
              {1, .8f, 1, .6f}};
        std::string error;
        auto        shader =
            [h.draw createModelShader:
                        "float3 alloy3dShade(ModelSurface s,float4 p){return s.litColor*p.rgb;}"
                          diagnostics:error];
                                 ...
                                 "animated_blend"})
        {
          ...
          for (bool custom : {false, true})
          {
            auto submit = [&]
            {
              ...
              [h.draw setModelTextureTransform:(alloy3d::ModelTextureTransform3D{{1.4f, .8f},
                                                                                 {.12f, .2f}})];
              [h.draw drawModelInstances:model instances:instances];
              [h.draw drawModel:blue
                       position:{0, 0, .45f}
                       rotation:{}
                          scale:{.8f, .8f, 1}
                          color:{1, 1, 1, 1}];
            ...
            auto scalar    = [h.draw modelDrawCallCount];
            [h.draw setTransparentBatching:true];
            auto batched = h.Run(camera, submit);
            ...
            if (model.parts.count == 1)
              Check(calls < 20, "single-part transparency did not batch around intervening model");
        ...
        auto submitStyles = [&]
        {
          for (int i = 0; i < 4; ++i)
          {
            [h.draw setModelShader:shader
                        parameters:(simd_float4{i % 2 ? 1.f : 0.f, 0, i % 2 ? 0.f : 1.f, 1})];
            [h.draw drawModel:blue
                     position:{0, 0, .4f}
                     rotation:{}
                        scale:{1, 1, 1}
        ...
        [h.draw setTransparentBatching:false];
        auto reference = h.Run(camera, submitStyles);
        [h.draw setTransparentBatching:true];
        Check(reference == h.Run(camera, submitStyles), "batch mixed shader parameters");
