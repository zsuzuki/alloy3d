  ...
  @autoreleasepool
  {
    try
    {
      ...
      if (!device) return 77;
      {
        ...
        Check((mapped[center] & 255) > (reference[center] & 255) + 35, "normal map did not change lighting correctly");
        [h.draw setModelNormalMapping:{0}];
        Check(h.Run(camera, [&] { submit(tilted); }) == reference, "disabled mapping changed flat rendering");
        [h.draw setModelNormalMapping:{1}];
        for (float value : {-1.f, 9.f, std::numeric_limits<float>::quiet_NaN()})
        {
          bool rejected = false;
          try { [h.draw setModelNormalMapping:(alloy3d::ModelNormalMapping3D{value})]; }
          catch (const std::invalid_argument &) { rejected = true; }
          Check(rejected && mapped == h.Run(camera, [&] { submit(tilted); }), "invalid strength changed state");
        }
        for (const char *kind : {"zero_scale", "degenerate_uv"})
        {
          auto model = h.Load(root / (std::string(kind)+".glb"));
          Check(h.Run(camera, [&] { submit(model); }) == reference, "zero/degenerate normal mapping changed base normal");
          [model release];
        }
        std::string error;
        ...
        Check(std::abs(int((normal[center]>>16)&255)-204)<3 && std::abs(int(normal[center]&255)-230)<3,
        ...
        Check(std::abs(int((mirroredNormal[center]>>16)&255)-51)<3, "mirrored UV handedness is wrong");
        [mirrored release];
        for (const char *kind : {"tilted", "tangent", "skin", "blend", "mask", "unlit"})
        {
          auto model = h.Load(root / (std::string(kind)+".glb"));
          auto shared = [model newInstance];
          Check(shared.parts[0].normalTexture == model.parts[0].normalTexture, "instance duplicated normal texture");
          ...
          for (float mirror : {-1.f,1.f})
          {
            ...
            auto individual = h.Run(camera,[&] {
              for (const auto &i : placements)
                [h.draw drawModel:model position:i.position rotation:i.rotation scale:i.scale color:i.color];
            });
            Check(individual == h.Run(camera,[&] { [h.draw drawModelInstances:model instances:placements]; }),
                  "mapped mirrored/scaled/skinned instancing differs");
          }
          [shared release]; [model release];
        }
        [flat release]; [tilted release];
      }
      [device release];
      std::puts("normal mapping: linear texture, lighting, disable, UV/tangent, degenerate, custom, shared, mirrored/skinned instances passed");
      return 0;
    }
