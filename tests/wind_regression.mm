  ...
  @autoreleasepool
  {
    try
    {
        ...
        return 77;
      {
        ...
        std::string         error;
        auto                shader = [h.draw
            createModelShader:"float3 alloy3dShade(ModelSurface s,float4 p){return s.normal*.5+.5;}"
                  diagnostics:error];
        Check(bool(shader), error.c_str());
        for (const char *name :
             {"single_sided", "mask_checker", "mirrored_skin", "animated_blend", "wind_strip"})
        {
          ...
          for (bool custom : {false, true})
          {
            ...
            [h.draw setModelWind:alloy3d::ModelWind3D{}];
            auto                 still = h.Run(camera, submit);
            alloy3d::ModelWind3D wind;
            wind.strength  = .4f;
            wind.time      = 1;
            wind.tipHeight = 1;
            [h.draw setModelWind:wind];
            auto bent = h.Run(camera, submit);
            Check(still != bent, "wind did not deform model or normals");
            Check(bent == h.Run(camera,
                                [&]
                                {
                                  [h.draw drawModelInstances:model
                                                   instances:std::span(&instance, 1)];
                                }),
                  "instanced wind differs from scalar");
            ...
            if (std::string(name) == "wind_strip")
              Check(std::equal(still.begin() + 160 * 256, still.end(), bent.begin() + 160 * 256),
                    "wind moved anchored base");
            wind.baseHeight = 100;
            wind.tipHeight  = 101;
            [h.draw setModelWind:wind];
            ...
            wind = {};
            [h.draw setModelWind:wind];
            Check(still == h.Run(camera, submit), "wind disable did not restore model");
        ...
        for (int y = 60; y < 130; ++y)
          for (int x = 164; x < 208; ++x)
            shadowChanges += first[y * 256 + x] != next[y * 256 + x];
