...
int main(int argc, char **argv)
{
  @autoreleasepool
  {
    try
    {
      ...
      Check(alloy3d::SelectLod3D(.1f, thresholds).nearLevel == 2,
            "small object retained full detail");
      auto mid = alloy3d::SelectLod3D(.5f, thresholds);
      Check(mid.nearLevel == 0 && mid.farLevel == 1 && std::abs(mid.farWeight - .5f) < 1e-5f,
            "LOD midpoint incorrect");
      alloy3d::CameraData camera;
      camera.buildPerspective(1, 1, .1f, 100);
      float size = alloy3d::ProjectedHeight3D(camera, {0, 0, -10}, 1);
      Check(std::abs(size - 1.f / (10 * std::tan(.5f))) < 1e-5f, "invalid size");
      Check(std::abs(alloy3d::ProjectedHeight3D(camera, {0, 0, -20}, 1) * 2 - size) < 1e-5f,
            "size did not scale with depth");
      camera.buildOrthographic(10, 1, .1f, 100);
      Check(std::abs(alloy3d::ProjectedHeight3D(camera, {0, 0, -20}, 1) - .2f) < 1e-5f,
            "orthographic screen size incorrect");
      ...
      for (NSUInteger samples : {1u, 4u})
      {
        ...
        for (const char *name : {"opaque_alpha", "mask_checker", "blend_red"})
        {
          ...
          for (bool custom : {false, true})
          {
            ...
            auto     full = h.Run(
                camera, [&] { [h.draw drawModelInstances:model instances:std::span(&base, 1)]; });
            for (float split : {0.f, .1f, .5f, .9f, 1.f})
            {
              std::array<Instance, 2> pair = {base, base};
              pair[0].coverage             = {0, split};
              pair[1].coverage             = {split, 1};
              for (bool batching : {false, true})
              {
                [h.draw setTransparentBatching:batching];
                Check(full ==
                          h.Run(camera, [&] { [h.draw drawModelInstances:model instances:pair]; }),
                      "complementary dither changed pixels");
              }
              Check(full == h.Run(camera,
                                  [&]
                                  {
                                    for (auto &i : pair)
                                    {
                                      [h.draw setModelCoverage:i.coverage];
            ...
            auto empty = h.Run(camera,
                               [&]
                               {
                                 [h.draw setModelCoverage:(simd_float2{0, 0})];
                                 [h.draw drawModel:model
            ...
            Check(std::all_of(empty.begin(), empty.end(), [](auto p) { return p == 0; }),
                  "empty coverage drew pixels");
            [h.draw setModelCoverage:(simd_float2{0, 1})];
          }
          [model release];
        }
        ...
        camera.buildOrthographic(4, 1, .1f, 20);
        [h.draw setModelShader:{} parameters:{}];
        [h.draw
            setDirectionalLight:(alloy3d::DirectionalLight3D{{.8f, 0, -1}, {1, 1, 1}, .2f, .8f})];
        ...
        std::array<Instance, 2> pair = {base, base};
        pair[0].coverage             = {0, .375f};
        pair[1].coverage             = {.375f, 1};
        Check(full == h.Run(camera, [&] { submit(pair); }), "dither changed shadow coverage");
        bool rejected = false;
        try
        {
          [h.draw setModelCoverage:(simd_float2{.8f, .2f})];
        }
