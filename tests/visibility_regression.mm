...
int main(int argc,char **argv)
{
  @autoreleasepool
  {
    try
    {
      ...
      Check(alloy3d::Frustum3D(camera).intersects({{-1,-1,-1},{1,1,1}}),"rotated camera lost target");
      auto device=MTLCreateSystemDefaultDevice();if(!device)return 77;
      {
        Harness h(device,argv[1]); auto root=std::filesystem::path(argv[2]);
        alloy3d::DirectionalShadow3D shadow;shadow.enabled=true;
        [h.draw setDirectionalShadow:shadow];[h.draw setTransparentBatching:true];
        for (const char *name : {"opaque_alpha","mask_checker","blend_parts","animated_blend"})
        {
          ...
          for(int i=0;i<60;++i)
            instances.push_back({{float(i%10)*4-18,float(i/10)*3-7,0},{0,float(i)*.1f,0},
                                {i%2 ? -.9f : .9f,1,1},{1,1,1,1}});
          for(bool instanced : {false,true})
          {
            auto submit=[&]{
              if(instanced) [h.draw drawModelInstances:model instances:instances];
              else for(auto &i:instances) [h.draw drawModel:model position:i.position rotation:i.rotation scale:i.scale color:i.color];
            };
            [h.draw setFrustumCulling:false];auto reference=h.Run(camera,submit);
            auto shadows=[h.draw shadowDrawCallCount];auto calls=[h.draw modelDrawCallCount];
            [h.draw setFrustumCulling:true];Check(reference==h.Run(camera,submit),"culling changed visible pixels or shadows");
