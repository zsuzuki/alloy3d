  ...
  @autoreleasepool
  {
    try
    {
        ...
        return 77;
      {
        ...
        auto                 plane = [&]
        {
          [h.draw drawPlane:{-4, -4, 0}
                         p1:{4, -4, 0}
                         p2:{4, 4, 0}
                         p3:{-4, 4, 0}
                      color:{1, 1, 1, 1}];
        };
        auto baseline = h.Run(camera, plane);
        [h.draw setHeightFog:fog];
        auto result = h.Run(camera, plane);
        Check((result[192 * 256 + 128] & 255) + 50 < (result[64 * 256 + 128] & 255),
              "fog did not become denser at lower world height");
        int expected = std::round(255 * std::exp(-.1f * 10));
        Check(std::abs(int(result[128 * 256 + 128] & 255) - expected) < 3,
              "horizontal ray did not integrate exponential density");
        // Rotate the camera and put the same world point at the center; world up must not rotate
        // with it.
        ...
        Check(std::abs(int(tilted[128 * 256 + 128] & 255) - expected) < 4,
              "height fog used camera-space height");
        fog.falloff    = 0;
        fog.maxOpacity = .3f;
        [h.draw setHeightFog:fog];
        Check(std::abs(int(h.Run(camera, plane)[128 * 256 + 128] & 255) - 179) < 2,
              "fog opacity limit failed");
        ...
        for (const char *name : {"opaque_alpha", "mask_checker", "blend_red", "unlit"})
        {
          ...
          auto submit = [&]
          { [h.draw drawModel:model position:{} rotation:{} scale:{2, 2, 2} color:{1, 1, 1, 1}]; };
          [h.draw setModelShader:{} parameters:{}];
          auto standard = h.Run(camera, submit);
          [h.draw setModelShader:shader parameters:{}];
          Check(standard == h.Run(camera, submit), "custom fog differs");
          Instance placement{{}, {}, {2, 2, 2}, {1, 1, 1, 1}};
          Check(standard == h.Run(camera,
                                  [&]
                                  {
                                    [h.draw drawModelInstances:model
                                                     instances:std::span(&placement, 1)];
                                  }),
                "instanced fog differs");
          [h.draw setHeightFog:{}];
          auto off = h.Run(camera, submit);
          [h.draw setHeightFog:fog];
          for (size_t i = 0; i < off.size(); ++i)
            Check((off[i] >> 24) == (standard[i] >> 24), "height fog changed material alpha");
          [model release];
        }
      }
      [device release];
      std::puts("height fog: integral, world up, cap, off, invalid input, custom/material/instance "
                "alpha passed");
      return 0;
    }
