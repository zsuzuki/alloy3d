...
int main(int argc, char **argv)
{
  @autoreleasepool
  {
    try
    {
      ...
      {
        bool rejected = false;
        try { SelectSampleCount(n, [](auto) { return true; }); }
      ...
      for (NSUInteger samples : {1u, 2u, 4u, 8u})
      {
        ...
        auto triangle = h.Run(camera, [&] {
          [h.draw drawTriangle:{-.8f, -.7f, .5f} p1:{.7f, -.6f, .5f}
                           p2:{-.3f, .8f, .5f} color:{1, 1, 1, 1}];
        });
        size_t partial = 0;
        for (auto pixel : triangle)
          if ((pixel & 255) > 0 && (pixel & 255) < 255) ++partial;
        ...
        for (auto model : {opaque, mask, blend})
        {
          ...
          auto reference = h.Run(camera, [&] { [h.draw drawModelInstances:model instances:instances]; });
          [h.draw setModelShader:shader parameters:{}];
          auto custom = h.Run(camera, [&] { [h.draw drawModelInstances:model instances:instances]; });
