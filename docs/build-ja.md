...
ビューアには既存の3モデルと、25・100・200関節のテストモデルを同梱しています。
起動時は展示全体を外側から見下ろすカメラになり、後列のテストモデルはそれぞれ
Wave・Lift・ブレンドを自動再生します。

- 矢印キー / ゲームパッド右スティック: 展示中央を軸にカメラを周回
- R: 初期視点・透視投影に戻す
- O: 透視投影 / 正射影を切り替える
- F: 表示中のモデル全体が入るよう、現在の姿勢・配置からカメラを調整する
- M: 不透明・切り抜き・半透明、片面／両面、Lit／Unlitを比較するマテリアル展示
- C: 標準／カスタムシェーダーによるトゥーン調の陰影を切り替える
- H: 平行光源の影をON/OFF（ビューワーでは既定でON）
- WASD / 左スティック: 手前のバウンサーを移動
- I: 81個のインスタンス描画と、資源を共有する3体の独立アニメーションのデモへ切替
