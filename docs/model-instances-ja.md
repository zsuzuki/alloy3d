...
OPAQUE/MASKは登録順で描画し、その後に半透明パーツを奥から手前へ並べます。
すべてのパーツがOPAQUEまたはMASK、かつすべての配置色のアルファが1以上なら、
テクスチャの有無によらず一括描画します。パーツ→配列内の個体の順になります。
BLENDのパーツ、またはアルファ1未満の配置を含む呼び出しは個別描画へ戻し、
他のモデルとまとめてソートします。1パーツだけのモデルもこの対象です。

この判定は[基本マテリアル対応](materials-ja.md)で更新しました。
以前の「テクスチャ付き複数パーツは常に個別、1パーツなら半透明も一括」という制約を置き換えます。
...
- 半透明のフォールバックと、OPAQUEのテクスチャ付きパーツの一括描画。
